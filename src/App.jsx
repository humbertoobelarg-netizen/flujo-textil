...
async function enviarEmailPedido(pedido) {
  try {
    const prendas = (pedido.prendas||[]).filter(p=>p.tipoPrenda).map((p,i)=>`Prenda ${i+1}: ${p.tipoPrenda} | $${parseFloat(p.precioUnit||0).toLocaleString("es-AR")} x ${p.cantidad||0} = $${((parseFloat(p.precioUnit)||0)*(parseFloat(p.cantidad)||0)).toLocaleString("es-AR")}`).join("\n");
    const totalGral = (pedido.prendas||[]).reduce((s,p)=>s+(parseFloat(p?.precioUnit)||0)*(parseFloat(p?.cantidad)||0),0);
...
function PrendaDetalle({ prenda, idx }) {
    ...
    <div style={{ border:"1.5px solid #d8d0c0",marginBottom:6 }}>
      <div onClick={()=>setAbierto(!abierto)} style={{ padding:"8px 12px",display:"flex",alignItems:"center",justifyContent:"space-between",cursor:"pointer",background:"#fef3ee" }}>
        <span style={{ fontFamily:"'Bebas Neue',sans-serif",fontSize:14,letterSpacing:1,color:"#e85d26" }}>
          PRENDA {idx+1} {prenda.tipoPrenda ? `— ${prenda.tipoPrenda}` : ""} {total>0?`· $${total.toLocaleString("es-AR")}`:""} 
        </span>
...
// ── RESUMEN PRECIOS ───────────────────────────────────────────
function ResumenPrecios({ prendas, anticipo }) {
    ...
    <div style={{ background:"#1a1208",color:"#f5f0e8",padding:"12px 14px",marginTop:8 }}>
      ...
      {ant > 0 && <>
        ...
        <div style={{ display:"flex",justifyContent:"space-between",borderTop:"1px solid #3a3a3a",paddingTop:8 }}>
          ...
          <span style={{ fontSize:15,fontWeight:600,color:"#e85d26" }}>${(totalGral-ant).toLocaleString("es-AR")}</span>
        </div>
      </>}
    </div>
  );
...
export default function App() {
  ...
  const [editandoPedido, setEditandoPedido] = useState(null); // pedido being edited
                    ...
                    <div key={pedidoActual.id} className="card fade" style={{ marginBottom:12,overflow:"hidden",borderLeft:`4px solid ${prog===100?"#10b981":prog>0?"#f59e0b":pri?.color}` }}>
                      <div style={{ padding:"14px 16px" }}>
                        ...
                        {(pedidoActual.prendas||[]).filter(pr=>pr.tipoPrenda).map((pr,i) => (
                          <PrendaDetalle key={i} prenda={pr} idx={i} />
                        ))}
                        <ResumenPrecios prendas={pedidoActual.prendas||[]} anticipo={pedidoActual.anticipo} />
          ...
          <div style={{ flex:1,overflowY:"auto",padding:20 }}>
            ...
            {adminTab === "pedidos" && (
              <div>
                      ...
                      {open && (
                        <div style={{ padding:"0 18px 16px",borderTop:"1.5px solid #e8e0d0" }} onClick={e=>e.stopPropagation()}>
                          ...
                          <div style={{ marginTop:10 }}>
                            ...
                            <ResumenPrecios prendas={p.prendas||[]} anticipo={p.anticipo} />
                          </div>
                ...
                {usuario?.rol === "admin" && pedidos.length > 0 && (() => {
                  ...
                  return (
                    <div style={{ marginTop:20,padding:"16px",background:"#1a1208",color:"#f5f0e8" }}>
                      ...
                      <div style={{ fontSize:10,color:"#8a7a6a",letterSpacing:1,marginBottom:8 }}>POR MES (fecha de entrega)</div>
          ...
          <div className="modal" onClick={e=>e.stopPropagation()}>
            ...
            <div style={{ padding:24,display:"flex",flexDirection:"column",gap:14 }}>
              ...
              {calcTotalGral(formEditar.prendas) > 0 && (
                <div>
                  <ResumenPrecios prendas={formEditar.prendas} anticipo={formEditar.anticipo} />
          ...
          <div className="modal" onClick={e=>e.stopPropagation()}>
            ...
            <div style={{ padding:24,display:"flex",flexDirection:"column",gap:14 }}>
              ...
              {calcTotalGral(formPedido.prendas) > 0 && (
                <div>
                  <ResumenPrecios prendas={formPedido.prendas} anticipo={formPedido.anticipo} />
                  <div style={{ marginTop:8 }}>
                    <label style={{ fontSize:10,letterSpacing:1,color:"#8a7a6a",display:"block",marginBottom:5 }}>ANTICIPO</label>
