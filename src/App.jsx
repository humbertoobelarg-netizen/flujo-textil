    ...
    await fetch("https://api.emailjs.com/api/v1.0/email/send", {
      ...
      body: JSON.stringify({
        ...
        template_params: {
          ...
          tipo_tejido: pedido.tipo_tejido || "-",
          molderia: pedido.molderia || "-",
          cuerpo_color: pedido.cuerpo || "-",
          manga_tipo: pedido.manga || "-",
          manga_color: pedido.color || "-",
          puno: pedido.puno || "-",
          cuello_tipo: pedido.cuello || "-",
          color_cuello: pedido.color_cuello || "-",
          cantidad: pedido.cantidad,
          prioridad: pedido.prioridad,
          fecha_entrega: formatFecha(pedido.fecha_entrega),
          creado_por: pedido.creado_por || "-",
          descripcion: pedido.descripcion || "-",
          talles: pedido.talles ? Object.entries(pedido.talles).filter(([k,v])=>parseInt(v)>0).map(([k,v])=>`${k}: ${v}`).join(" | ") : "-",
...
const FORM_INIT = { cliente:"", prioridad:"media", fechaEntrega:"", descripcion:"", procesosActivos:["orden","terminacion"], tipoPrenda:"", tipoTejido:"", molderia:"", talles:{}, cuerpo:"", manga:"", color:"", puno:"", cuello:"", colorCuello:"", imagenes:[], precios:[{precioUnit:"",cantidad:"",concepto:""},{precioUnit:"",cantidad:"",concepto:""},{precioUnit:"",cantidad:"",concepto:""}], anticipo:"" };
...
export default function App() {
  ...
  async function handleLogin() {
    ...
    if (u.rol === "admin" || u.nombre === "Vivi") {
      setPantalla("admin");
    } else {
      setPantalla("operario");
    }
  }
  ...
  async function subirImagenes(pedidoId, imagenes) {
    ...
    for (let i = 0; i < imagenes.length; i++) {
      ...
      const path = `${pedidoId}/imagen_${timestamp}_${i+1}.${ext}`;
      try {
        ...
        if (res.ok) {
          ...
          showToast("Error al subir imagen", "#ef4444");
        }
  ...
  async function crearPedido() {
    if (!formPedido.cliente || !formPedido.fechaEntrega || !formPedido.cuerpo || !formPedido.manga || !formPedido.color || !formPedido.puno || !formPedido.cuello || !formPedido.colorCuello) return;
    const procesos = {};
    PROCESOS.forEach(p => { procesos[p.key] = "pendiente"; });
    if (procesos["orden"] !== undefined) procesos["orden"] = "listo";
    const cantidad = Object.values(formPedido.talles||{}).reduce((sum,v)=>sum+(parseInt(v)||0),0);
    const nuevo = {
      id: newId(pedidos),
      ...
      molderia: formPedido.molderia||"",
      cuerpo: formPedido.cuerpo||"",
      manga: formPedido.manga||"",
      color: formPedido.color||"",
      puno: formPedido.puno||"",
      cuello: formPedido.cuello||"",
      color_cuello: formPedido.colorCuello||"",
      precios: formPedido.precios||[],
          ...
          <div style={{ flex:1,padding:16,overflowY:"auto" }}>
            {(() => {
              ...
              const misPedidos = pedidos.filter(p => {
                ...
                // Todos los usuarios de orden solo ven sus propios pedidos
                if (miProceso === "orden") {
              ...
              // Vista especial para usuarios de orden - muestran progreso general
              if (miProceso === "orden") {
                      ...
                      <div style={{ padding:"14px 16px" }}>
                        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8 }}>
                          <div>
                            ...
                            <div style={{ fontSize:11,color:"#8a7a6a",marginTop:2 }}>{pedidoActual.id} · {pedidoActual.cantidad} unidades</div>
                          </div>
                        ...
                        <div style={{ marginBottom:10,display:"grid",gridTemplateColumns:"1fr 1fr",gap:6 }}>
                          {pedidoActual.tipo_prenda && <div style={{ padding:"6px 10px",background:"#f5f0e8" }}><div style={{ fontSize:9,color:"#8a7a6a",letterSpacing:1 }}>TIPO DE PRENDA</div><div style={{ fontSize:12,fontWeight:500 }}>{pedidoActual.tipo_prenda}</div></div>}
                          {pedidoActual.tipo_tejido && <div style={{ padding:"6px 10px",background:"#f5f0e8" }}><div style={{ fontSize:9,color:"#8a7a6a",letterSpacing:1 }}>TIPO DE TEJIDO</div><div style={{ fontSize:12,fontWeight:500 }}>{pedidoActual.tipo_tejido}</div></div>}
                          {pedidoActual.molderia && <div style={{ padding:"6px 10px",background:"#f5f0e8" }}><div style={{ fontSize:9,color:"#8a7a6a",letterSpacing:1 }}>MOLDERÍA</div><div style={{ fontSize:12,fontWeight:500 }}>{pedidoActual.molderia}</div></div>}
                          {pedidoActual.cuerpo && <div style={{ padding:"6px 10px",background:"#f5f0e8" }}><div style={{ fontSize:9,color:"#8a7a6a",letterSpacing:1 }}>CUERPO COLOR</div><div style={{ fontSize:12,fontWeight:500 }}>{pedidoActual.cuerpo}</div></div>}
                          {pedidoActual.manga && <div style={{ padding:"6px 10px",background:"#f5f0e8" }}><div style={{ fontSize:9,color:"#8a7a6a",letterSpacing:1 }}>MANGA TIPO</div><div style={{ fontSize:12,fontWeight:500 }}>{pedidoActual.manga}</div></div>}
                          {pedidoActual.color && <div style={{ padding:"6px 10px",background:"#f5f0e8" }}><div style={{ fontSize:9,color:"#8a7a6a",letterSpacing:1 }}>MANGA COLOR</div><div style={{ fontSize:12,fontWeight:500 }}>{pedidoActual.color}</div></div>}
                          {pedidoActual.puno && <div style={{ padding:"6px 10px",background:"#f5f0e8" }}><div style={{ fontSize:9,color:"#8a7a6a",letterSpacing:1 }}>PUÑO</div><div style={{ fontSize:12,fontWeight:500 }}>{pedidoActual.puno}</div></div>}
                          {pedidoActual.cuello && <div style={{ padding:"6px 10px",background:"#f5f0e8" }}><div style={{ fontSize:9,color:"#8a7a6a",letterSpacing:1 }}>CUELLO TIPO</div><div style={{ fontSize:12,fontWeight:500 }}>{pedidoActual.cuello}</div></div>}
                          {pedidoActual.color_cuello && <div style={{ padding:"6px 10px",background:"#f5f0e8" }}><div style={{ fontSize:9,color:"#8a7a6a",letterSpacing:1 }}>COLOR CUELLO</div><div style={{ fontSize:12,fontWeight:500 }}>{pedidoActual.color_cuello}</div></div>}
                        </div>
                        {pedidoActual.talles && Object.keys(pedidoActual.talles).some(k=>parseInt(pedidoActual.talles[k])>0) && (
                          <div style={{ marginBottom:10,padding:"8px 10px",background:"#f5f0e8" }}>
                            <div style={{ fontSize:9,color:"#8a7a6a",letterSpacing:1,marginBottom:6 }}>TALLES</div>
                            <div style={{ display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:4 }}>
                              {Object.entries(pedidoActual.talles).filter(([k,v])=>parseInt(v)>0).map(([talle,cant])=>(
                                <div key={talle} style={{ textAlign:"center",padding:"4px",background:"#fff",border:"1px solid #d8d0c0" }}>
                                  <div style={{ fontSize:9,color:"#8a7a6a",fontWeight:600 }}>{talle}</div>
                                  <div style={{ fontSize:13,fontWeight:600,color:"#1a1208" }}>{cant}</div>
                                </div>
                              ))}
                            </div>
                          </div>
                        )}
                        {pedidoActual.descripcion && <div style={{ fontSize:12,color:"#5a4a3a",marginBottom:10,padding:"8px 10px",background:"#f5f0e8",borderLeft:"3px solid #c8bfaf" }}>{pedidoActual.descripcion}</div>}
                        {(pedidoActual.imagenes_urls||[]).length > 0 && (
                          <div style={{ marginBottom:10,padding:"8px 10px",background:"#f5f0e8" }}>
                            <div style={{ fontSize:9,color:"#8a7a6a",letterSpacing:1,marginBottom:8 }}>IMÁGENES DEL DISEÑO</div>
                            <div style={{ display:"flex",gap:8,flexWrap:"wrap" }}>
                              {(pedidoActual.imagenes_urls||[]).map((url,i) => (
                                <a key={i} href={url} target="_blank" rel="noreferrer">
                                  <img src={url} alt={`Diseño ${i+1}`} style={{ width:80,height:80,objectFit:"cover",border:"1.5px solid #d8d0c0" }} />
                                </a>
                              ))}
                            </div>
                          </div>
                    ...
                    return (
                      <div key={pedidoActual.id} className="card fade" style={{ marginBottom:12,overflow:"hidden",borderLeft:`4px solid ${etapa==="listo"?"#10b981":etapa==="en_proceso"?"#f59e0b":pri?.color}` }}>
                        <div style={{ padding:"14px 16px" }}>
                          ...
                          {(pedidoActual.imagenes_urls||[]).length > 0 && (
                            <div style={{ marginBottom:10,padding:"8px 10px",background:"#f5f0e8" }}>
                              ...
                              <div style={{ display:"flex",gap:8,flexWrap:"wrap" }}>
                                {(pedidoActual.imagenes_urls||[]).map((url,i) => (
                                  <a key={i} href={url} target="_blank" rel="noreferrer">
                                    <img src={url} alt={`Diseño ${i+1}`} style={{ width:90,height:90,objectFit:"cover",border:"1.5px solid #d8d0c0" }} />
                                  </a>
                                ))}
                              </div>
                            </div>
                          ...
                          <div style={{ fontSize:11,color:"#8a7a6a",marginBottom:12 }}>📅 Entrega: {pedidoActual.fecha_entrega}</div>
                          {miProceso === "diseno" && (
                            <div style={{ marginBottom:10 }}>
                              <label style={{ fontSize:10,letterSpacing:1,color:"#8a7a6a",display:"block",marginBottom:6 }}>SUBIR IMÁGENES DEL DISEÑO (máx. 3)</label>
          ...
          <div style={{ flex:1,overflowY:"auto",padding:20 }}>
            ...
            {adminTab === "pedidos" && (
              <div>
                      ...
                      {open && (
                        <div style={{ padding:"0 18px 16px",borderTop:"1.5px solid #e8e0d0" }} onClick={e=>e.stopPropagation()}>
                          ...
                          <div style={{ marginTop:10,display:"grid",gridTemplateColumns:"1fr 1fr",gap:6 }}>
                            {p.tipo_prenda && <div style={{ padding:"6px 10px",background:"#f5f0e8" }}><div style={{ fontSize:9,color:"#8a7a6a",letterSpacing:1 }}>TIPO DE PRENDA</div><div style={{ fontSize:12 }}>{p.tipo_prenda}</div></div>}
                            {p.tipo_tejido && <div style={{ padding:"6px 10px",background:"#f5f0e8" }}><div style={{ fontSize:9,color:"#8a7a6a",letterSpacing:1 }}>TIPO DE TEJIDO</div><div style={{ fontSize:12 }}>{p.tipo_tejido}</div></div>}
                            {p.molderia && <div style={{ padding:"6px 10px",background:"#f5f0e8" }}><div style={{ fontSize:9,color:"#8a7a6a",letterSpacing:1 }}>MOLDERÍA</div><div style={{ fontSize:12 }}>{p.molderia}</div></div>}
                            {p.cuerpo && <div style={{ padding:"6px 10px",background:"#f5f0e8" }}><div style={{ fontSize:9,color:"#8a7a6a",letterSpacing:1 }}>CUERPO COLOR</div><div style={{ fontSize:12 }}>{p.cuerpo}</div></div>}
                            {p.manga && <div style={{ padding:"6px 10px",background:"#f5f0e8" }}><div style={{ fontSize:9,color:"#8a7a6a",letterSpacing:1 }}>MANGA TIPO</div><div style={{ fontSize:12 }}>{p.manga}</div></div>}
                            {p.color && <div style={{ padding:"6px 10px",background:"#f5f0e8" }}><div style={{ fontSize:9,color:"#8a7a6a",letterSpacing:1 }}>MANGA COLOR</div><div style={{ fontSize:12 }}>{p.color}</div></div>}
                            {p.puno && <div style={{ padding:"6px 10px",background:"#f5f0e8" }}><div style={{ fontSize:9,color:"#8a7a6a",letterSpacing:1 }}>PUÑO</div><div style={{ fontSize:12 }}>{p.puno}</div></div>}
                            {p.cuello && <div style={{ padding:"6px 10px",background:"#f5f0e8" }}><div style={{ fontSize:9,color:"#8a7a6a",letterSpacing:1 }}>CUELLO TIPO</div><div style={{ fontSize:12 }}>{p.cuello}</div></div>}
                            {p.color_cuello && <div style={{ padding:"6px 10px",background:"#f5f0e8" }}><div style={{ fontSize:9,color:"#8a7a6a",letterSpacing:1 }}>COLOR CUELLO</div><div style={{ fontSize:12 }}>{p.color_cuello}</div></div>}
                          </div>
                          {p.talles && Object.keys(p.talles).some(k=>parseInt(p.talles[k])>0) && (
                            <div style={{ marginTop:10,padding:"8px 12px",background:"#f5f0e8" }}>
                              <div style={{ fontSize:9,color:"#8a7a6a",letterSpacing:1,marginBottom:8 }}>TALLES</div>
                              <div style={{ display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:4 }}>
                                {Object.entries(p.talles).filter(([k,v])=>parseInt(v)>0).map(([talle,cant])=>(
                                  <div key={talle} style={{ textAlign:"center",padding:"4px",background:"#fff",border:"1px solid #d8d0c0" }}>
                                    <div style={{ fontSize:9,color:"#8a7a6a",fontWeight:600 }}>{talle}</div>
                                    <div style={{ fontSize:13,fontWeight:600 }}>{cant}</div>
                                  </div>
                                ))}
                              </div>
                            </div>
                          )}
                          {(p.precios||[]).some(pr=>parseFloat(pr?.precioUnit)>0) && (
                            <div style={{ marginTop:8,padding:"8px 12px",background:"#f5f0e8" }}>
                              <div style={{ fontSize:9,color:"#8a7a6a",letterSpacing:1,marginBottom:8 }}>PRECIOS</div>
                              {(p.precios||[]).map((pr,i) => {
                                const t = (parseFloat(pr?.precioUnit)||0)*(parseFloat(pr?.cantidad)||0);
                                return t > 0 ? (
                                  <div key={i} style={{ display:"flex",justifyContent:"space-between",marginBottom:4,fontSize:12 }}>
                                    <span>Precio {i+1}: ${parseFloat(pr.precioUnit).toLocaleString("es-AR")} x {pr.cantidad}</span>
                                    <span style={{ fontWeight:600 }}>${t.toLocaleString("es-AR")}</span>
                                  </div>
                                ) : null;
                              })}
                              {(() => {
                                const tg = (p.precios||[]).reduce((s,pr)=>s+(parseFloat(pr?.precioUnit)||0)*(parseFloat(pr?.cantidad)||0),0);
                                const ant = parseFloat(p.anticipo)||0;
                                return (
                                  <div style={{ borderTop:"1px solid #d8d0c0",marginTop:6,paddingTop:6 }}>
                                    <div style={{ display:"flex",justifyContent:"space-between",fontSize:12,fontWeight:600 }}>
                                      <span>TOTAL GENERAL</span><span>${tg.toLocaleString("es-AR")}</span>
                                    </div>
                                    {ant > 0 && <>
                                      <div style={{ display:"flex",justifyContent:"space-between",fontSize:12,color:"#8a7a6a" }}>
                                        <span>Anticipo</span><span>-${ant.toLocaleString("es-AR")}</span>
                                      </div>
                                      <div style={{ display:"flex",justifyContent:"space-between",fontSize:12,fontWeight:600,color:"#e85d26" }}>
                                        <span>SALDO</span><span>${(tg-ant).toLocaleString("es-AR")}</span>
                                      </div>
                                    </>}
                                  </div>
                                );
                            ...
                            <div style={{ marginTop:8,padding:"8px 12px",background:"#f5f0e8" }}>
                              ...
                              <div style={{ display:"flex",gap:8,flexWrap:"wrap" }}>
                                {(p.imagenes_urls||[]).map((url,i) => (
                                  <a key={i} href={url} target="_blank" rel="noreferrer">
                                    <img src={url} alt={`Diseño ${i+1}`} style={{ width:100,height:100,objectFit:"cover",border:"1.5px solid #d8d0c0" }} />
                                  </a>
                                ))}
                              </div>
                            </div>
                          )}
                          {usuario?.rol === "admin" && <div style={{ marginTop:12,display:"flex",justifyContent:"flex-end" }}>
                ...
                {!pedidos.length && <div style={{ padding:40,textAlign:"center",color:"#b0a898",fontSize:13 }}>No hay pedidos. Tocá + PEDIDO para crear uno.</div>}
              </div>
            )}
                  ...
                  {PROCESOS.map(proc => {
                    ...
                    return (
                      <div key={proc.key} style={{ flex:1,minWidth:140 }}>
                        ...
                        {enProceso.length > 0 && (
                          <div style={{ marginBottom:4,padding:"3px 8px",background:"#f59e0b15",borderBottom:"2px solid #f59e0b33" }}>
                            <span style={{ fontSize:9,letterSpacing:1,color:"#f59e0b",fontWeight:600 }}>EN PROCESO</span>
                          </div>
                        )}
                        {enProceso.map(p => {
                        ...
                        {listos.length > 0 && (
                          <div style={{ marginBottom:4,padding:"3px 8px",background:"#10b98115",borderBottom:"2px solid #10b98133" }}>
                            <span style={{ fontSize:9,letterSpacing:1,color:"#10b981",fontWeight:600 }}>LISTOS</span>
                          </div>
                        )}
                        {listos.map(p => (
          ...
          <div className="modal" onClick={e=>e.stopPropagation()}>
            ...
            <div style={{ padding:24,display:"flex",flexDirection:"column",gap:14 }}>
              ...
              <div>
                ...
                <input type="text" style={{ width:"100%" }} placeholder="Ej: Remeras, buzos..." value={formPedido.tipoPrenda||""} onChange={e=>setFormPedido({...formPedido,tipoPrenda:e.target.value})} />
              </div>
              <div>
                <label style={{ fontSize:10,letterSpacing:1,color:"#8a7a6a",display:"block",marginBottom:5 }}>TIPO DE TEJIDO</label>
                <input type="text" style={{ width:"100%" }} placeholder="Ej: Algodón, poliéster..." value={formPedido.tipoTejido||""} onChange={e=>setFormPedido({...formPedido,tipoTejido:e.target.value})} />
              </div>
              <div>
                <label style={{ fontSize:10,letterSpacing:1,color:"#8a7a6a",display:"block",marginBottom:5 }}>MOLDERÍA</label>
                <input type="text" style={{ width:"100%" }} placeholder="Moldería..." value={formPedido.molderia||""} onChange={e=>setFormPedido({...formPedido,molderia:e.target.value})} />
              </div>
              <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:12 }}>
                <div>
                  <label style={{ fontSize:10,letterSpacing:1,color:"#8a7a6a",display:"block",marginBottom:5 }}>CUERPO COLOR *</label>
                  <input type="text" style={{ width:"100%" }} placeholder="Color del cuerpo..." value={formPedido.cuerpo||""} onChange={e=>setFormPedido({...formPedido,cuerpo:e.target.value})} />
                </div>
                <div>
                  <label style={{ fontSize:10,letterSpacing:1,color:"#8a7a6a",display:"block",marginBottom:5 }}>MANGA TIPO *</label>
                  <input type="text" style={{ width:"100%" }} placeholder="Tipo de manga..." value={formPedido.manga||""} onChange={e=>setFormPedido({...formPedido,manga:e.target.value})} />
                </div>
                <div>
                  <label style={{ fontSize:10,letterSpacing:1,color:"#8a7a6a",display:"block",marginBottom:5 }}>MANGA COLOR *</label>
                  <input type="text" style={{ width:"100%" }} placeholder="Color de manga..." value={formPedido.color||""} onChange={e=>setFormPedido({...formPedido,color:e.target.value})} />
                </div>
                <div>
                  <label style={{ fontSize:10,letterSpacing:1,color:"#8a7a6a",display:"block",marginBottom:5 }}>PUÑO *</label>
                  <input type="text" style={{ width:"100%" }} placeholder="Puño..." value={formPedido.puno||""} onChange={e=>setFormPedido({...formPedido,puno:e.target.value})} />
                </div>
                <div>
                  <label style={{ fontSize:10,letterSpacing:1,color:"#8a7a6a",display:"block",marginBottom:5 }}>CUELLO TIPO *</label>
                  <input type="text" style={{ width:"100%" }} placeholder="Tipo de cuello..." value={formPedido.cuello||""} onChange={e=>setFormPedido({...formPedido,cuello:e.target.value})} />
                </div>
                <div>
                  <label style={{ fontSize:10,letterSpacing:1,color:"#8a7a6a",display:"block",marginBottom:5 }}>COLOR CUELLO *</label>
                  <input type="text" style={{ width:"100%" }} placeholder="Color cuello..." value={formPedido.colorCuello||""} onChange={e=>setFormPedido({...formPedido,colorCuello:e.target.value})} />
                </div>
              </div>
              <div>
                <label style={{ fontSize:10,letterSpacing:1,color:"#8a7a6a",display:"block",marginBottom:8 }}>TALLES Y CANTIDADES</label>
                <div style={{ display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:6 }}>
                  {TALLES_LIST.map(talle => (
                    <div key={talle} style={{ display:"flex",flexDirection:"column",alignItems:"center",gap:4 }}>
                      ...
                      <input type="number" min="0" style={{ width:"100%",textAlign:"center",padding:"6px 4px",fontSize:12 }}
                        ...
                        onChange={e => setFormPedido(prev => ({...prev, talles:{...prev.talles, [talle]: e.target.value}}))}
                      />
                    </div>
                  ))}
                </div>
              ...
              <div style={{ borderTop:"1.5px solid #d8d0c0",paddingTop:14 }}>
                <label style={{ fontSize:10,letterSpacing:1,color:"#8a7a6a",display:"block",marginBottom:10 }}>PRECIOS</label>
                {[0,1,2].map(i => {
                  const p = (formPedido.precios||[])[i]||{};
                  const total = (parseFloat(p.precioUnit)||0) * (parseFloat(p.cantidad)||0);
                  return (
                    <div key={i} style={{ marginBottom:10,padding:"10px",background:"#fff",border:"1.5px solid #d8d0c0" }}>
                      <div style={{ fontSize:10,color:"#8a7a6a",letterSpacing:1,marginBottom:8 }}>PRECIO {i+1}</div>
                      <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:6 }}>
                        <div>
                          <label style={{ fontSize:9,color:"#8a7a6a",display:"block",marginBottom:3 }}>PRECIO UNITARIO</label>
                          <input type="number" min="0" style={{ width:"100%",padding:"8px" }} placeholder="0.00"
                            value={p.precioUnit||""}
                            onChange={e => {
                              const ps = [...(formPedido.precios||[{},{},{}])];
                              ps[i] = {...ps[i], precioUnit: e.target.value};
                              setFormPedido(prev=>({...prev,precios:ps}));
                            }}
                          />
                        </div>
                        <div>
                          <label style={{ fontSize:9,color:"#8a7a6a",display:"block",marginBottom:3 }}>CANTIDAD</label>
                          <input type="number" min="0" style={{ width:"100%",padding:"8px" }} placeholder="0"
                            value={p.cantidad||""}
                            onChange={e => {
                              const ps = [...(formPedido.precios||[{},{},{}])];
                              ps[i] = {...ps[i], cantidad: e.target.value};
                              setFormPedido(prev=>({...prev,precios:ps}));
                            }}
                          />
                        </div>
                      </div>
                      {total > 0 && <div style={{ fontSize:12,fontWeight:600,color:"#1a1208",textAlign:"right" }}>Total: ${total.toLocaleString("es-AR")}</div>}
                    </div>
                  );
                })}
                {(() => {
                  const totalGral = (formPedido.precios||[]).reduce((sum,p)=>{
                    return sum + (parseFloat(p?.precioUnit)||0)*(parseFloat(p?.cantidad)||0);
                  },0);
                  const anticipo = parseFloat(formPedido.anticipo)||0;
                  const saldo = totalGral - anticipo;
                  return totalGral > 0 ? (
                    <div style={{ padding:"10px 14px",background:"#1a1208",color:"#f5f0e8",marginBottom:8 }}>
                      <div style={{ display:"flex",justifyContent:"space-between",marginBottom:6 }}>
                        <span style={{ fontSize:11,letterSpacing:1 }}>TOTAL GENERAL</span>
                        <span style={{ fontSize:14,fontWeight:600 }}>${totalGral.toLocaleString("es-AR")}</span>
                      </div>
                      <div style={{ marginBottom:8 }}>
                        <label style={{ fontSize:9,color:"#b0a898",display:"block",marginBottom:4 }}>ANTICIPO</label>
                        <input type="number" min="0" style={{ width:"100%",padding:"8px",background:"#2a2a2a",border:"1px solid #3a3a3a",color:"#f5f0e8",fontSize:13 }} placeholder="0.00"
                          value={formPedido.anticipo||""}
                          onChange={e=>setFormPedido(prev=>({...prev,anticipo:e.target.value}))}
                        />
                      </div>
                      {anticipo > 0 && (
                        <div style={{ display:"flex",justifyContent:"space-between",borderTop:"1px solid #3a3a3a",paddingTop:8 }}>
                          ...
                          <span style={{ fontSize:14,fontWeight:600,color:"#e85d26" }}>${saldo.toLocaleString("es-AR")}</span>
                        </div>
                      )}
                    </div>
                  ) : null;
              ...
              <div>
                ...
                <input type="file" accept="image/*" multiple style={{ width:"100%",background:"#f5f0e8",border:"1.5px solid #c8bfaf",padding:"8px",fontSize:12,color:"#1a1208" }}
                  onChange={e => {
                    const files = Array.from(e.target.files).slice(0,3);
                    setFormPedido(prev => ({...prev, imagenes: files}));
                  }}
                />
                {(formPedido.imagenes||[]).length > 0 && (
                  <div style={{ display:"flex",gap:8,marginTop:8,flexWrap:"wrap" }}>
                    {(formPedido.imagenes||[]).map((f,i) => (
                      <div key={i} style={{ fontSize:11,color:"#8a7a6a",background:"#f5f0e8",padding:"4px 8px",border:"1px solid #c8bfaf" }}>
                        📎 {f.name}
                      </div>
                    ))}
                  </div>
