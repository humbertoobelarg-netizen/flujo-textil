...
function newId() { return "P" + Date.now().toString().slice(-6); }
        ...
        <div style={{ maxWidth:480,margin:"0 auto",minHeight:"100vh",display:"flex",flexDirection:"column" }}>
          ...
          <div style={{ flex:1,padding:16,overflowY:"auto" }}>
            {(() => {
              ...
              const misPedidos = pedidos.filter(p => (p.procesos_activos||[]).includes(miProceso));
              ...
              return sorted.map(p => {
                const etapa = (p.procesos||{})[miProceso] || "pendiente";
                const pri = PRIORIDADES.find(pr=>pr.key===p.prioridad);
                const vencido = p.fecha_entrega < hoy() && etapa !== "listo";
                    ...
                    <div key={p.id} className="card fade" style={{ marginBottom:10,overflow:"hidden",cursor:"pointer",borderLeft:`4px solid ${pri?.color}` }} onClick={() => setSelectedPedido(open?null:p)}>
                      ...
                      {open && (
                        <div style={{ padding:"0 18px 16px",borderTop:"1.5px solid #e8e0d0" }} onClick={e=>e.stopPropagation()}>
                          ...
                          {p.descripcion && <div style={{ marginTop:10,fontSize:12,color:"#5a4a3a",padding:"8px 12px",background:"#f5f0e8",borderLeft:"3px solid #c8bfaf" }}>{p.descripcion}</div>}
