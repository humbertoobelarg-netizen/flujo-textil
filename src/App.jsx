...
function PrendaDetalle({ prenda, idx, showTejido=false }) {
                    ...
                    <div key={pedidoActual.id} className="card fade" style={{ marginBottom:12,overflow:"hidden",borderLeft:`4px solid ${prog===100?"#10b981":prog>0?"#f59e0b":pri?.color}` }}>
                      <div style={{ padding:"14px 16px" }}>
                        ...
                        {(pedidoActual.prendas||[]).filter(pr=>pr.tipoPrenda).map((pr,i) => (
                          <PrendaDetalle key={i} prenda={pr} idx={i} showTejido={usuario?.nombre==="Vivi" || usuario?.nombre==="Gabi"} />
                        ))}
                    ...
                    return (
                      <div key={pedidoActual.id} className="card fade" style={{ marginBottom:12,overflow:"hidden",borderLeft:`4px solid ${borderColor}`,background:bgColor }}>
                        <div style={{ padding:"14px 16px" }}>
                          ...
                          {/* Prendas */}
                          {(pedidoActual.prendas||[]).filter(pr=>pr.tipoPrenda).map((pr,i) => (
                            <PrendaDetalle key={i} prenda={pr} idx={i} showTejido={usuario?.rol==="admin" || usuario?.nombre==="Vivi" || usuario?.nombre==="Gabi"} />
                          ))}
                          {(pedidoActual.imagenes_urls||[]).length > 0 && (
            ...
            <div style={{ padding:24,display:"flex",flexDirection:"column",gap:14 }}>
              ...
              <div>
                ...
                <input type="file" accept="image/*" multiple style={{ width:"100%",background:"#f5f0e8",border:"1.5px solid #c8bfaf",padding:"8px",fontSize:12,color:"#1a1208" }}
                  onChange={e => setFormPedido(prev => ({...prev, imagenes: A
