    ...
    <div style={{ border:"1.5px solid #d8d0c0",marginBottom:6 }}>
      <div onClick={()=>setAbierto(!abierto)} style={{ padding:"8px 12px",display:"flex",alignItems:"center",justifyContent:"space-between",cursor:"pointer",background:"#fef3ee" }}>
        <span style={{ fontFamily:"'Bebas Neue',sans-serif",fontSize:14,letterSpacing:1,color:"#e85d26" }}>
          PRENDA {idx+1} {prenda.tipoPrenda ? `— ${prenda.tipoPrenda}` : ""} {showPrecios && total>0?`· $${total.toLocaleString("es-AR")}`:""} 
        </span>
...
function puedeVerPrecios(usuario, pedido=null) {
  if (usuario?.rol === "admin") return true;
  if (usuario?.nombre === "Vivi" || usuario?.nombre === "Gabi" || usuario?.nombre === "Romina") return true;
  // Usuario de orden que creó el pedido también puede ver
  if (pedido && usuario?.proceso === "orden" && pedido.creado_por === usuario?.nombre) return true;
  return false;
}
                ...
                <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:6 }}>
                  {PROCESOS.map(proc => {
                    ...
                    return (
                      <div key={proc.key} className={`checkbox-proceso${active?" active":""}`}
                        onClick={() => {
                          const next = active ? formPedido.procesosActivos.filter(k=>k!==
