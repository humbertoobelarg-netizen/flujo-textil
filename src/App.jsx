...
function puedeVerPrecios(usuario) {
  return usuario?.rol === "admin" || usuario?.nombre === "Vivi" || usuario?.nombre === "Gabi";
}
                    ...
                    <div key={pedidoActual.id} className="card fade" style={{ marginBottom:12,overflow:"hidden",borderLeft:`4px solid ${prog===100?"#10b981":prog>0?"#f59e0b":pri?.color}` }}>
                      <div style={{ padding:"14px 16px" }}>
                        ...
                        {(pedidoActual.prendas||[]).filter(pr=>pr.tipoPrenda).map((pr,i) => (
                          <PrendaDetalle key={i} prenda={pr} idx={i} showTejido={puedeVerPrecios(usuario)} />
                        ))}
                        {puedeVerPrecios(usuario) && <ResumenPrecios prendas={pedidoActual.prendas||[]} anticipo={pedidoActual.anticipo} pagos={pedidoActual.pagos||[]} />}
